...
{
    [BurstCompile]
    public struct AllJoinJob : IJobExt<NativeArray<Vector3>, NativeArray<int>>
    {
        ...
        public void Execute()
        {
            for (int z = 0, v = 0, vi = 0, t = 0; z <= DataSize; z++)
            {
                ...
                for (int x = 0; x <= DataSize; x++, v++)
                {
                    ...
                    _vertices[v] = new Vector3(x * _stepSize - 0.5f, noiseValue, z * _stepSize - 0.5f);
                    if (x < DataSize && z < DataSize)
                    {
                        _triangles[t] = vi;
                        _triangles[t + 1] = vi + DataSize + 1;
                        _triangles[t + 2] = vi + 1;
                        _triangles[t + 3] = vi + 1;
                        _triangles[t + 4] = vi + DataSize + 1;
                        _triangles[t + 5] = vi + DataSize + 2;
                        t += 6;
                        vi++;
                    }
                }
