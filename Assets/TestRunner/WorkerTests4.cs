...
{
    public static class WorkerTests<T1, T2, T3, T4>
    {
        #region TestFacade

        internal static ITestFacade TestFacadeInstance<TWorker, TWorkConfig, TDataConfig>(string testName,
            IInputData<T1, T2, T3, T4> data, TWorkConfig workConfig, TDataConfig[] dataConfig)
            where TWorker : struct, IWorker<T1, T2, T3, T4>
            where TWorkConfig : class, IWorkConfig
            where TDataConfig : class, IDataConfig
        {
            return new WorkerWrapper<TWorker, TWorkConfig, T1, T2, T3, T4>(workConfig).WorkerWrapper(testName, data,
                dataConfig);
        }
        ...
        public static ITestFacade Run<TWorker>(string testName, Array itemArray1, Array itemArray2, Array itemArray3,
            ...
            where TWorker : struct, IWorker<T1, T2, T3, T4>
        {
            var data = new InputData<T1, T2, T3, T4>(itemArray1, itemArray2, itemArray3, itemArray4);
            try
            {
