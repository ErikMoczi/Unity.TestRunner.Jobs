...
namespace WorkSpace.Tests.Basic.Addition.Simd
{
    public sealed class TripleTypeDouble2Test : SampleGenerator
    {
        ...
        public override IWorkWrapper[] InitWorkWrappers(IInputDataContainer inputDataContainer, int dataSize)
        {
            return new[]
            {
                WorkerTests<double2, double2, double2>.RunIJob(TestName, new SimdAdditionDouble2Job(),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    new WorkConfigIJob(Allocator.Persistent, true)),
                WorkerTests<double2, double2, double2>.RunIJobParallelFor(TestName, new SimdAdditionDouble2JobParallelFor(),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
