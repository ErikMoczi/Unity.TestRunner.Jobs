...
namespace WorkSpace.Tests.Basic.Addition.Simd
{
    public sealed class TripleTypeDouble4Test : SampleGenerator
    {
        ...
        public override IWorkWrapper[] InitWorkWrappers(IInputDataContainer inputDataContainer, int dataSize)
        {
            return new[]
            {
                WorkerTests<double4, double4, double4>.RunIJob(TestName, new SimdAdditionDouble4Job(),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
                    new WorkConfigIJob(Allocator.Persistent, true)),
                WorkerTests<double4, double4, double4>.RunIJobParallelFor(TestName, new SimdAdditionDouble4JobParallelFor(),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
                    inputDataContainer.GetData<double4>(DataConfig.DataDouble4),
