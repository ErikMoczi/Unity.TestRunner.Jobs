...
namespace WorkSpace.Tests.Basic.Division.Simd
{
    public sealed class TripleTypeDouble3Test : SampleGenerator
    {
        ...
        public override IWorkWrapper[] InitWorkWrappers(IInputDataContainer inputDataContainer, int dataSize)
        {
            return new[]
            {
                WorkerTests<double3, double3, double3>.RunIJob(TestName, new SimdDivisionDouble3Job(),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
                    new WorkConfigIJob(Allocator.Persistent, true)),
                WorkerTests<double3, double3, double3>.RunIJobParallelFor(TestName, new SimdDivisionDouble3JobParallelFor(),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
                    inputDataContainer.GetData<double3>(DataConfig.DataDouble3),
