...
namespace WorkSpace.Tests.Basic.Multiplication.Simd.Optimalization
{
    public sealed class TripleTypeDouble2Test : SampleGenerator
    {
        ...
        public override IWorkWrapper[] InitWorkWrappers(IInputDataContainer inputDataContainer, int dataSize)
        {
            return new[]
            {
                WorkerTests<double2, double2, double2>.RunIJob(TestName, new SimdMultiplicationOptimalizationDouble2Job(),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    new WorkConfigIJob(Allocator.Persistent, true)),
                WorkerTests<double2, double2, double2>.RunIJobParallelFor(TestName,
                    new SimdMultiplicationOptimalizationDouble2JobParallelFor(),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
                    inputDataContainer.GetData<double2>(DataConfig.DataDouble2),
